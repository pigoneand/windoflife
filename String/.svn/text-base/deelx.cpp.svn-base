...
void init()
{
	CRegexpT<char> regexp("\\w+");

	/** 严格匹配 */
	MatchResult result1 = regexp.MatchExact("helloworldabc");
	cout << result1.IsMatched() << endl;
	...
	while (true)
	{
		MatchResult result = regexp.Match(query.data(), startoff);
		if (!result.IsMatched()) break;

		string matched = query.substr(result.GetStart(), result.GetEnd() - result.GetStart());
		cout << matched << endl;
		startoff = result.GetEnd();
	}
}
