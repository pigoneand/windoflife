...
{
public:
    bool isDanger(int s1, int s2)
    {
        return (s1 >= 2 * s2 && s1 <= 10 * s2);
    }
    
    bool isDangerTwo(int s1, int s2)
    {
        return isDanger(s1, s2) || isDanger(s2, s1);
    }
    ...
    int peaceful(int minSize, int maxSize, vector <int> fishSizes)
    {
        ...
        for (int i = minSize; i <= maxSize; ++i)
        {
            ...
            for (int j = 0; j < fishSizes.size(); ++j)
                if (isDangerTwo(i, fishSizes[j])) ok = false;
            if (ok) ans++;
        }
        return ans;
    }
